...
>    所有題目的核心函式（灰階轉換、CDF 直方圖等化查表、四點透視投影動態長寬推算、自動多邊形角點偵測、SIFT 特徵動態匹配與動態畫布拼接）皆為**完全動態實作**，支援任意尺寸、解析度與拍攝視角之影像。
> 2. **嚴格分開「基礎題（15%）」與「進階題（10%）」**：
>    每道題目均嚴格對齊課程簡報（第 57~60 頁）規範，清清楚楚標註基礎題與進階題的實作內容與評分點，方便逐項比對給分。
...
├── Output\                    # 產出成果與可視化圖表
│   ├── quiz1_comparison.png   # 灰階對比圖與數值誤差分析
│   ├── quiz2_comparison.png   # 等化前後直方圖分佈與統計量
│   ├── quiz3_comparison.png   # 透視校正與多角度自動偵測成果
│   ├── quiz4_comparison.png   # 特徵匹配、拼接與多條件壓測分析
│   └── quiz4_panorama_blended.jpg # 全景拼接圖
...
*圖 1：彩色原圖、OpenCV 灰階圖、NumPy 灰階圖與兩者絕對誤差熱圖 (Difference Heatmap)。*
...
### 5.3 視覺化成果
![Quiz 4 SIFT 特徵拼接與多條件壓測成果](Output/quiz4_comparison.png)
*圖 4.1：SIFT 特徵匹配（上）、基礎直接覆蓋接縫 vs 進階無痕漸變融合（中）、50° 極端旋轉幾何畸變失效畫面 vs CLAHE 前處理增強暗部特徵匹配（下）。*

![Quiz 4 全景圖](Output/quiz4_panorama_blended.jpg)
*圖 4.2：進階題距離權重線性羽化無痕拼接全景成果圖。*

### 5.4 壓力測試數據分析 (動態即時運算評測)
